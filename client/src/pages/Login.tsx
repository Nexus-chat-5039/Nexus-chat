...
export default function Login() {
  ...
  const handleRequestResetOtp = async () => {
    try {
      ...
      setResetError(e instanceof Error ? e.message : "Failed to send code")
    } finally {
  ...
  const handleVerifyResetOtp = async () => {
    try {
      ...
      setResetError(e instanceof Error ? e.message : "Invalid code")
    } finally {
      setResetLoading(false)
    }
  }
    ...
    <AuthLayout title="Login" subtitle="Welcome back">
      <input
        ...
        className="
          ...
          outline-none
          focus:border-nexus-primary/50 focus:ring-1 focus:ring-nexus-primary/20
          transition-all
        "
      />
      ...
      {error && (
        <p className="mb-3 text-sm text-red-400 bg-red-500/5 border border-red-500/10 rounded-xl px-3 py-2">{error}</p>
      )}
      ...
      <button
        ...
        className="
          w-full rounded-xl bg-nexus-primary py-3 font-semibold text-white
      ...
      <a
        ...
        className="
          ...
          border border-nexus-border py-3 font-medium transition-all
          hover:bg-nexus-hover active:scale-[0.98]
        "
      >
      ...
      <Modal isOpen={showResetModal} onClose={() => setShowResetModal(false)} title="Reset Password">
        <div className="space-y-4">
          {resetError && <p className="text-sm text-red-400 bg-red-500/5 border border-red-500/10 rounded-xl px-3 py-2">{resetError}</p>}

          {resetState === "email" && (
            <>
              <p className="text-sm text-nexus-muted">Enter your email address to receive a verification code.</p>
              <input
                className="w-full rounded-xl bg-nexus-input px-4 py-3 text-nexus-text border border-nexus-border focus:border-nexus-primary/50 outline-none text-sm transition-all"
              ...
              <button
                ...
                className="w-full bg-nexus-primary py-2.5 rounded-xl font-medium text-white hover:brightness-110 disabled:opacity-50 transition-all"
              >
          ...
          {resetState === "otp" && (
            <>
              <p className="text-sm text-nexus-muted">Enter the 6-digit code sent to <strong>{resetEmail}</strong></p>
              <input
                className="w-full rounded-xl bg-nexus-input px-4 py-3 text-nexus-text border border-nexus-border focus:border-nexus-primary/50 outline-none text-center tracking-widest text-lg transition-all"
                placeholder="000000"
                maxLength={6}
                value={resetOtp}
